...
const Results = ({ snippet, keywords, prompt, handleBack }: Props) => {
  ...
  return (
    <div className="flex flex-col gap-2 items-center">
      ...
      <h3 className="text-lg">Snippet for <span className="text-slate-700">{prompt}</span>:</h3>
      <CopyToClipboard
      ...
      >
        <button
        disabled={copied}
        className={`${copied ?"bg-white/20":""} absolute top-1 right-3  hover:bg-white/20 rounded-xl p-1 transition duration-100  `}
      ...
      {keywords.map((word) => (
        <div className="bg-white/10 px-2 py-1 rounded-full hover:bg-white/30 transition-all duration-150 cursor-pointer">#{word}</div>
      ))}
      </div>
      </div>
      <button
